...
Процедура ТестДолжен_ПроверитьСрабатыванияПроверки() Экспорт
	...
	Проверка = Новый ХардкодСетевогоАдресов;
	ИмяФайла = ОбъединитьПути(".", "tests", "checks", "ХардкодСетевогоАдресов.os");
	...
	Ожидаем.Что(ТаблицаОшибок.Количество(), "Количество замечаний должено быть равно 4").Равно(4);
